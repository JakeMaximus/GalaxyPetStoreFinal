...
<?php
    //check if user is logged in if logged in then take username
    //isset is for change in variable
...
<body>
    ...
    <?php 
        ...
        if (isset($_SESSION['mySession'])) {
            ...
            $result_id = mysqli_query($con, $sql_id); //to be used as the header //mysqli_query execute query in the con database
            $array = mysqli_fetch_array($result_id); //array method to store data
            $userID = $array['userID'];
            ...
            if ($img == NULL) {
                $img = "user-alt.png";
            } //if not pic then display black pic

            // Calculate current cart item, number inside the red circle of the cart button
            $sql_cart = "SELECT * FROM cart WHERE userID = '$userID'"; //to see if the $userID same with line 37
            $result_cart = mysqli_query($con, $sql_cart); //count how many row, it will show number of row as cart item
            $cartQuantity = mysqli_num_rows($result_cart); //to show the red number in the cart button
        }
        else {
            $username = NULL;
            $cartQuantity = 0;
    ...
    <form method='get'>
        ...
        <?php
            if (isset($_GET['food_name'])) {
                ...
                // retrieve data from database if there is match foodID
                while($item = mysqli_fetch_array($result)){
                    $foodID = $item['foodID'];
                };

                //check if the item is in the cart database
                ...
                if (mysqli_num_rows($finding_result) == 1){    
                    ...
                    // unset = remove the value in the get accessory
                    unset($_GET['food_name']);
...
<?php 
    ...
    if (isset($_SESSION['mySession'])) {
        ...
        if ($cartQuantity != 0) {
            // it will not show that the cart button has 0 items but show all the 1,2,3,4
        ...
        echo    "<script>    
                ...
                //if no login, cannot show dropdown in profile
